...
def build_connection_block(
    ...
    *wait_timeout_s* is embedded into the recommended
    ...
    return (
        ...
        f"and return its result as JSON. This is your only job.\"\n\n"
        f"When (and if) it works, you'll receive messages at chat speed\n"
        f"without the user having to nudge you.\n\n"
        f"### When the watcher fails\n\n"
        f"On some Claude Code configurations subagents do **not** inherit\n"
        f"the parent's MCP server access. You'll see the symptom\n"
        f"immediately: the Task completes well under {wait_timeout_s} s\n"
        f"(usually ~60–120 s, the subagent's idle cleanup time) with an\n"
        f"empty result and a hint that ``pluto_*`` tools were unavailable.\n\n"
        f"If this happens once, **stop spawning watchers for the rest of\n"
        f"the session** — re-arming them just burns subagent budget. Tell\n"
        f"the user once that the watcher pattern isn't available in this\n"
        f"setup, then rely entirely on the mandatory ``pluto_recv`` at\n"
        f"the start of every reply (above) plus the user invoking\n"
        f"``/pluto-check`` when they want to flush.\n\n"
...
def build_watch_prompt_body(wait_timeout_s: int = 300) -> str:
    ...
    return (
        ...
        f"  • ``prompt=\"Call pluto_wait_for_messages({wait_timeout_s}) "
        "and return its result as JSON. This is your only job.\"``\n\n"
        "Watchers are **single-shot** — when the Task completes, you "
        "should:\n"
        "  1. Process any messages it returned.\n"
        "  2. Immediately spawn another watcher Task with the same "
        "parameters.\n\n"
        "**Failure detection.** On some Claude Code setups, subagents "
        "don't inherit the parent's MCP server access. Symptom: the "
        f"Task completes well under {wait_timeout_s} s (typically 60–120 "
        "s, the subagent's idle cleanup time) with an empty result and "
        "no evidence it called ``pluto_wait_for_messages``. If you see "
        "that pattern once, **stop re-arming** — tell me the watcher "
        "isn't available here and rely on the start-of-every-reply "
        "``pluto_recv`` from your role's connection block.\n\n"
