...
def build_connection_block(
    ...
    *wait_timeout_s* is embedded into the recommended
    ...
    return (
        ...
        f"## ⚡ MANDATORY FIRST ACTION — start an inbox watcher\n\n"
        f"**Before you do anything else this session — before you greet the\n"
        f"user, before you confirm your role, before you read any file —\n"
        f"spawn a background Pluto inbox watcher.** Use the Task tool with:\n\n"
        ...
        f"Without this watcher you will **not receive any messages** from\n"
        f"other agents until the user types something to you. Other agents\n"
        f"on this team rely on your watcher being active.\n\n"
        f"When that watcher Task completes (it will appear in your next\n"
        f"turn's context), do exactly two things in order:\n\n"
        f"  1. Process any messages it returned.\n"
        f"  2. **Immediately spawn another watcher Task** with the same\n"
        f"     parameters. Watchers are single-shot — you must keep\n"
        f"     re-arming them or you go dark again.\n\n"
        ...
        f"   continuing.\n"
        f"2. **Pull (on demand).** ``pluto_recv`` drains the inbox\n"
        f"   immediately, no blocking.\n"
        f"3. The user can re-trigger the watcher manually at any time via\n"
        f"   the ``/pluto-watch`` slash command — useful if the chain\n"
        f"   breaks (you forgot to re-arm) or you want to confirm a\n"
        f"   watcher is active."
    )
...
def build_watch_prompt_body(wait_timeout_s: int = 300) -> str:
    ...
    return (
        ...
        "Watchers are **single-shot** — when the Task completes, you must:\n"
        "  1. Process any messages it returned.\n"
        "  2. **Immediately spawn another watcher Task** with the same "
        "parameters, or you go dark again.\n\n"
        "Reply briefly with ``watcher started`` once the first Task is in "
        "flight."
    )
