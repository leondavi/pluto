...
def build_connection_block(
    ...
    *iterations* sets how many short-poll cycles the watcher subagent
    ...
    return (
        ...
        f"```\n"
        f"You are a Pluto inbox watcher. Repeat this loop exactly as written:\n"
        ...
        f"the tool calls themselves are the output. Do NOT add any sleep,\n"
        f"delay, or pause commands between iterations — the blocking call\n"
        f"itself provides the wait.\n"
        f"```\n\n"
        f"Why this works: each ``pluto_wait_for_messages({wait_timeout_s})``\n"
        f"call **blocks** for up to {wait_timeout_s} s waiting for messages\n"
        f"to arrive, then returns immediately with what it has. There is no\n"
        ...
        f"### MANDATORY: respawn the watcher immediately after every drain\n\n"
        f"Whenever the inbox is drained — by **any** of these paths:\n\n"
        f"  • the watcher Task fires and returns messages, **or**\n"
        f"  • you call ``pluto_recv`` (turn-start or otherwise), **or**\n"
        f"  • messages arrive piggybacked on another tool result\n"
        f"    under ``_pluto_inbox``\n\n"
        f"…you MUST respawn a fresh watcher Task **in the same turn**, with\n"
        f"the same parameters as the original. Do NOT wait for the in-flight\n"
        f"watcher to fire again on its own, and do NOT assume one is still\n"
        f"alive — re-arm proactively. A drain that leaves no watcher in\n"
        f"flight is a chat-speed delivery gap; respawning is cheap and\n"
        f"idempotent (a still-running watcher will simply finish its loop\n"
...
def build_check_prompt_body() -> str:
    ...
    return (
        ...
        "**Respawn the watcher.** Whether the inbox was empty or not, "
        "this drain counts — immediately spawn a fresh background Pluto "
        "inbox watcher Task afterwards (same parameters as your role's "
        "default watcher), unless the ``watcher_stop`` kill switch is in "
...
def build_watch_prompt_body(
    ...
    return (
        ...
        "```\n"
        "You are a Pluto inbox watcher. Repeat this loop exactly as written:\n"
        f"  1. Call pluto_wait_for_messages({wait_timeout_s}).\n"
        ...
        "Do NOT add any sleep, delay, or pause commands — the blocking\n"
        "call itself provides the wait.\n"
        "```\n\n"
