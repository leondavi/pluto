...
def build_connection_block(
    ...
    *iterations* sets how many short-poll cycles the watcher subagent
    ...
    return (
        ...
        f"    watcher is in flight. If not, spawn one with the same\n"
        f"    parameters as the original.\n"
        ...
        f"    delivery point.\n\n"
        f"Why: server-side dedupe means a duplicate ``pluto_inbox_watch``\n"
        f"is cheap (it returns ``{{already_watching: true}}`` immediately),\n"
        f"but it still counts against your subagent budget. Re-arming at\n"
        f"end-of-turn keeps exactly one watcher in flight whenever you go\n"
        f"idle, without over-spawning during active turns.\n\n"
