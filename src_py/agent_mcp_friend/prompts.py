...
def build_connection_block(
    ...
    *iterations* sets how many short-poll cycles the watcher subagent
    ...
    return (
        ...
        f"## Pipeline / event-driven work — prefer ``pluto_pop``\n\n"
        f"The default delivery mode is **batch**: ``pluto_recv`` and the\n"
        f"``_pluto_inbox`` piggyback on every Pluto tool result return all\n"
        f"buffered messages at once. That is the right shape for\n"
        f"turn-driven / interactive work where you process whatever has\n"
        f"queued up since your last reply.\n\n"
        f"If you are doing **pipeline work** — every inbox message is a\n"
        f"discrete unit of work (a file to compile, a task to run, a stage\n"
        f"in a graph) and you want one message per turn so you can react\n"
        f"to each event individually — switch into single-pop mode and\n"
        f"drive consumption explicitly:\n\n"
        f"  1. Call ``pluto_set_delivery_mode(\"single\")`` once near the\n"
        f"     top of the session. From that point on, ``piggyback``\n"
        f"     attaches at most one message under ``_pluto_inbox`` plus a\n"
        f"     ``_pluto_inbox_remaining`` counter — unrelated Pluto tool\n"
        f"     calls no longer bulk-drain the buffer behind your back.\n"
        f"  2. On each watcher wake (or inbox notification), call\n"
        f"     ``pluto_pop(wait_s=<short>)``. Process the single message.\n"
        f"     If the response's ``remaining`` is > 0, call ``pluto_pop``\n"
        f"     again immediately; otherwise yield and wait for the next\n"
        f"     wake. ``pluto_pop`` blocks up to ``wait_s`` seconds for an\n"
        f"     arrival when the buffer is empty.\n"
        f"  3. ``pluto_recv`` still works in single-mode (it is a thin\n"
        f"     fallback that drains everything), but the canonical\n"
        f"     consumer is ``pluto_pop``.\n\n"
        f"Trade-off: single-pop adds one round-trip per message, which is\n"
        f"negligible for agent-paced traffic but a real cost at firehose\n"
        f"rates. Stay in batch mode unless per-message reactivity is the\n"
        f"point.\n\n"
        f"You can read the active mode from ``pluto_session`` under the\n"
        f"``delivery_mode`` field.\n\n"
