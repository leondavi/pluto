...
Agent: worker-alpha
═══════════════════
Performs 10 rounds of:
  1. Receive task assignment from coordinator
  2. Acquire primary + shared resource locks (nested locking)
  3. Implement feature, write to files
  4. Exchange sync message with worker-beta
  5. Cross-review worker-beta's work (acquires beta's file lock)
  6. Send progress/ack back to coordinator
  7. Update shared build log and scoreboard (lock contention with beta)
"""
    ...
    try:
        ...
        for rnd in range(1, rounds + 1):
            ...
            with open(path, "a") as f:
                f.write(f"[{MY_ID}] Round {rnd}: Implemented feature-{rnd}A\n")
            ...
            # Release shared first (reverse order — good lock discipline)
            ...
            review_out = os.path.join(WORK, f"review_r{rnd}_alpha.txt")
